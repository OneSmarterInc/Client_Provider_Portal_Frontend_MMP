...
const ProviderSettings = ({ isOpen, onClose }) => {
  const { api, providerNumbers, updateProviderNumbers } =
  ...
  const handleAddNpiToTaxId = async (taxId, npi, sequenceNumber, extraFields = {}) => {
    setIsAdding(true);
    try {
      await axios.post(
        `${api}/auth/providers/add/`,
        { provider_no: taxId, npi, provider_sequence: sequenceNumber || "", ...extraFields },
        { headers }
      );
